...
Phone:</p>
		...
		<div id="customer">
            <h3>Customer</h3>
            <p id="customer-title">{{$invoices_op[0]->client_name}}</p>

            <table id="meta">
            	<tr>
                    <td class="meta-head">Order No</td>
                    <td><span>{{$invoices_op[0]->order_no}}</span></td>
                </tr>
                <tr>
                    <td class="meta-head">Invoice #</td>
                    <td><span>{{$invoices_op[0]->invoice_code}}</span></td>
                </tr>
                <tr>

                    <td class="meta-head">Date</td>
                    <td><span id="date">{{$invoices_op[0]->created_at}}</span></td>
                </tr>
                <tr>
                    <td class="meta-head">Amount Due</td>
                    <td><div class="due">{{$invoices_op[0]->due_ammount}}</div></td>
                </tr>
...
?>
<tr class="item-row">
		      <td class="item-name"><div class="delete-wpr"><span>{{$row->product_name}}-{{$row->packet_name}}({{$row->packetsqty}}){{$row->unit_quantity}}{{$row->unit_name}}</span></div></td>
		      <td class="description"><span></span></td>
		      <td><span class="cost">{{$row->price}}</span></td>
		      <td><span class="qty">{{$row->quantity}}</span></td>
		      <td><span class="qty">{{$row->bonus}}</span></td>
		      <td><span class="price">{{$row->price*$row->quantity}}</span></td>
		  </tr>
...
@endforeach
				  ...
				  <tr>
				      ...
				      <td class="total-value"><div id="subtotal">{{$rt}}</div></td>
				  </tr>
				  <tr>

				      <td colspan="2" class="blank"> </td>
				      <td colspan="2" class="total-line">Total Vat</td>
				      <td class="total-value"><div id="total">{{$vati}} Taka</div></td>
				  </tr>
				  <tr>

				      <td colspan="2" class="blank"> </td>
				      <td colspan="2" class="total-line">Total With Vat</td>
				      <td class="total-value"><div id="total">{{$rt+$vati}} Taka</div></td>
				  </tr>
				  <tr>
				      <td colspan="2" class="blank"> </td>
				      <td colspan="2" class="total-line">Amount Paid</td>

				      <td class="total-value"><span id="paid">{{$total_w_vat}}</span></td>
				  </tr>
				  <tr>
				      <td colspan="2" class="blank"> </td>
				      <td colspan="2" class="total-line balance">Balance Due</td>
				      <td class="total-value balance"><div class="due">{{$invoices_op[0]->due_ammount}}</div></td>
				  </tr>
