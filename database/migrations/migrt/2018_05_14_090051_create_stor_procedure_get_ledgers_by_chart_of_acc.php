...
class CreateStorProcedureGetLedgersByChartOfAcc extends Migration
{
    /**
     * Run the migrations.
     *
...
SELECT mjp.*,
 CONCAT(mu.first_name,'(',mu.address,')') as client_details, 
 mjp.account_head_id as acc_final_name,
 CONCAT(pr.name,'(',pk.name,'(',pk.quantity,') ',un.name,'(',pk.unit_quantity,'))') as productDetails
 
FROM mxp_journal_posting mjp inner JOIN mxp_users mu ON(mu.user_id=mjp.client_id) 
