...
class TranslationController extends Controller
{
    ...
    public function manageTranslationKey($page = 1)
    {
        ...
        $totalData          =   DB::table('mxp_translations')->count();
        $get_translations   =   DB::select('call get_all_translation_with_limit('.$start.','.$this->limitPerPage.')');

        $translations       =   $this->getTranslationArray($get_translations);

        return view('trans.manageTranslation.index', [
